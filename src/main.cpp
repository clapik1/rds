...
double bump(double x, double y) {
    ...
    if(r < 0.125)
        return std::pow(std::cos(2. * std::acos(-1.) * r * 2.), 2);
    return 0.;
}
...
int main(int argc, char *argv[]) {
    ...
    bool animateFlag = args.optionExists("-a");
    ...
    auto t = 1.;
    auto iterTotal = static_cast<unsigned int>(std::ceil(t / maxDt));
    ...
    if(animateFlag) {
        ...
        mSolver.solve(t, zeroLambda, iterTotal);
        mMesh.toTecplot(ofs);
    }
    ofs.close();
